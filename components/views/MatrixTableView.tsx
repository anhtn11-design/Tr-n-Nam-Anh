...
export const MatrixTableView: React.FC<MatrixTableViewProps> = ({ ideas, onSelectIdea }) => {
  ...
  return (
    <div className="bg-brand-surface rounded-lg shadow-md overflow-hidden border border-brand-border">
        ...
        <table className="w-full text-sm text-left text-brand-text-muted">
          ...
          <tbody>
            {ideas.map((idea) => (
              <tr 
                key={idea.id} 
                className="bg-brand-surface border-b border-brand-border hover:bg-gray-50 cursor-pointer"
                onClick={() => onSelectIdea(idea)}
              >
                <th scope="row" className="px-6 py-4 font-medium text-brand-text-main whitespace-normal">
                  <p>{idea.upgradedTitle}</p>
                  <p className="text-xs text-brand-text-muted font-normal italic mt-1">Gốc: {idea.title}</p>
                </th>
                <td className="px-6 py-4">
                  <div className="flex flex-col gap-1">
                    <span className="px-2 py-1 text-xs font-medium text-blue-700 bg-blue-100 rounded-full w-fit">{idea.creativeStrategy}</span>
                    <span className="px-2 py-1 text-xs font-medium text-teal-700 bg-teal-100 rounded-full w-fit">{idea.variationType}</span>
                  </div>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <div className="text-xs font-bold text-brand-secondary mt-1">Viral: {idea.viralScore.toFixed(1)}</div>
                  </div>
                </td>
                <td className="px-6 py-4">{idea.implementationSuggestion}</td>
                <td className="px-6 py-4 text-brand-text-main italic truncate max-w-xs">"{getProminentHook(idea)}"</td>
              </tr>
            ))}
          </tbody>
        </table>
