...
export const Header: React.FC<HeaderProps> = ({ onGenerate, onRemix, setViewMode, activeViewMode, hasIdeas, onLogoClick, isRemixing, isRemixView }) => {
  ...
  return (
    <header className="flex-shrink-0 bg-brand-surface border-b border-brand-muted p-4 shadow-sm z-10">
      <div className="flex flex-col items-center justify-between gap-4">
        <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4 cursor-pointer" onClick={onLogoClick}>
            <div className="bg-brand-primary p-2 rounded-lg shadow-inner">
          ...
          <form onSubmit={handleSubmit} className="flex-grow md:max-w-xl w-full flex flex-col items-center gap-2">
            <div className="w-full flex items-center gap-2">
              <input
                ...
                className="w-full px-4 py-2 bg-brand-dark/50 border border-brand-muted rounded-lg focus:ring-2 focus:ring-brand-primary focus:outline-none transition"
              />
              <select 
                value={videoType} 
                onChange={(e) => setVideoType(e.target.value as VideoType)}
                className="px-3 py-2 bg-brand-dark/50 border border-brand-muted rounded-lg focus:ring-2 focus:ring-brand-primary focus:outline-none transition appearance-none"
              >
              ...
              <button type="submit" className="px-5 py-2 bg-brand-primary text-white font-semibold rounded-lg hover:bg-brand-primary-hover transition-colors duration-200 shadow-sm">
                Tạo ý tưởng
              </button>
            </div>
            <div className="w-full flex items-center justify-between mt-2">
                ...
                <div className="flex items-center gap-2 text-sm text-brand-text-muted">
                    ...
                    <input 
                        ...
                        className="w-20 md:w-24 cursor-pointer accent-brand-primary"
          ...
          {hasIdeas && (
            <div className="flex items-center gap-2 bg-brand-dark/50 p-1 rounded-lg">
        ...
        {hasIdeas && (
          <div className="w-full mt-2 p-4 bg-brand-dark/30 rounded-lg border border-brand-muted">
             <form onSubmit={handleRemixSubmit} className="flex flex-col md:flex-row items-center gap-3">
              ...
              <button type="submit" disabled={isRemixing} className="px-5 py-2 bg-brand-accent text-brand-text font-semibold rounded-lg hover:opacity-90 transition-opacity duration-200 disabled:bg-brand-muted disabled:cursor-not-allowed shadow-sm">
